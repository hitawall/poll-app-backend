...
// QueryPolloption chains the current query on the "polloption" edge.
func (vq *VoteQuery) QueryPolloption() *PollOptionQuery {
...
func (vq *VoteQuery) Clone() *VoteQuery {
	...
	return &VoteQuery{
		...
		withPolloption: vq.withPolloption.Clone(),
...
// WithPolloption tells the query-builder to eager-load the nodes that are connected to
// the "polloption" edge. The optional arguments are used to configure the query builder of the edge.
func (vq *VoteQuery) WithPolloption(opts ...func(*PollOptionQuery)) *VoteQuery {
	query := (&PollOptionClient{config: vq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	vq.withPolloption = query
	return vq
}

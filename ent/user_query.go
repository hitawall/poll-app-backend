...
func (uq *UserQuery) loadPolls(ctx context.Context, query *PollQuery, nodes []*User, init func(*User), assign func(*User, *Poll)) error {
	...
	for _, n := range neighbors {
		fk := n.user_polls
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_polls" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_polls" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil

...
// QueryPolloptions chains the current query on the "polloptions" edge.
...
// the "polloptions" edge. The optional arguments are used to configure the query builder of the edge.
...
func (pq *PollQuery) loadPolloptions(ctx context.Context, query *PollOptionQuery, nodes []*Poll, init func(*Poll), assign func(*Poll, *PollOption)) error {
	...
	query.Where(predicate.PollOption(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(poll.PolloptionsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.poll_polloptions
		if fk == nil {
			return fmt.Errorf(`foreign-key "poll_polloptions" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "poll_polloptions" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
